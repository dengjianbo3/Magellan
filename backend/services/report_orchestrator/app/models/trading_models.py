...
class TradingSignal(BaseModel):
    ...
    leverage: int = Field(ge=1, default=1)  # No hardcoded upper limit, validated dynamically
    ...
    def validate_leverage(cls, v: int) -> int:
        ...
        max_leverage_str = os.getenv("MAX_LEVERAGE", "20")
        try:
            max_leverage = int(max_leverage_str)
        except ValueError:
            max_leverage = 20
        if v > max_leverage:
...
class AgentVote(BaseModel):
    ...
    confidence: int = Field(ge=0, le=100)
    reasoning: str
    suggested_leverage: int = Field(ge=1)  # No hardcoded upper limit, validated dynamically
    suggested_tp_percent: float
    suggested_sl_percent: float

    @field_validator('suggested_leverage')
    @classmethod
    def validate_suggested_leverage(cls, v: int) -> int:
        """Validate suggested leverage against runtime config"""
        max_leverage_str = os.getenv("MAX_LEVERAGE", "20")
        try:
            max_leverage = int(max_leverage_str)
        except ValueError:
